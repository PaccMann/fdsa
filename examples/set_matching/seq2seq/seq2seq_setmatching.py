...
def main(model_path, results_path, training_params):
    ...
    logging.basicConfig(
        handlers=[
            logging.FileHandler(
                os.path.join(results_path, "set_matchingSeq2Seq.log")
            ),
    ...
    for cell in ["GRU", "nBRC", "LSTM", "BRC"]:
        ...
        for epoch in range(epochs):
            ...
            for idx, (
                ...
                train_loss = loss_fn(
                    train_output, targets21_train, targets12_train
                )
                ...
                if idx == (len(train_loader) - 2):
                    ...
                    all_correct = (
                        (train_preds == targets12_train)
                        .all(dim=1)
                        .float()
                        .sum()
                    )
                    correct_train = (
                        (train_preds == targets12_train).float().sum()
                    )
            ...
            for idx, (
                ...
                valid_loss = loss_fn(
                    valid_output, targets21_valid, targets12_valid
                )
                ...
                all_correct_valid += (
                    (valid_preds == targets12_valid).all(dim=1).float().sum()
                )
        ...
        for idx, (
            ...
            test_loss = loss_fn(test_output, targets21_test, targets12_test)
            ...
            all_correct_test += (
                (test_preds == targets12_test).all(dim=1).float().sum()
            )
