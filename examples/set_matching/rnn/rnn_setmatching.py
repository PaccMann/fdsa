...
parser.add_argument(
    "results_path", type=str, help="Path to save results and plots."
)
parser.add_argument(
...
def main(model_path: str, results_path: str, training_params: str):
    ...
    logging.basicConfig(
        handlers=[
            logging.FileHandler(
                os.path.join(results_path, "set_matchingRNN.log")
            ),
    ...
    for cell in ["GRU", "nBRC", "LSTM", "BRC"]:
        ...
        save_here = os.path.join(
            model_path, "{}_{}".format(params["loss"], cell)
        )
        model = None
        ...
        for epoch in range(epochs):
            ...
            for (
                ...
                train_x = torch.cat((x1_train, connector_, x2_train), dim=1).to(
                    device
                )
                ...
                train_loss = loss_fn(
                    train_output, target12_train, target21_train
                )
                optimzr.zero_grad()
                train_loss.backward()
                optimzr.step()
                ...
                if idx == (len(train_loader) - 2):
                    ...
                    all_correct = (
                        (train_preds == target21_train).all(dim=1).float().sum()
                    )
                    correct_train = (
                        (train_preds == target21_train).float().sum()
                    )
            ...
            for (
                ...
                valid_x = torch.cat((x1_valid, connector_, x2_valid), dim=1).to(
                    device
                )
                ...
                valid_loss = loss_fn(
                    valid_output, target12_valid, target21_valid
                )

                avg_valid_loss = (
                    avg_valid_loss * idx + valid_loss.detach()
                ) / (idx + 1)
        ...
        logger.info(
            "Total epochs time for {} : {} hours".format(
                cell, sum(epoch_times) / 3600
            )
        )

        torch.save(
            model_times, os.path.join(results_path, f"model_times_{cell}")
        )
        torch.save(
            epoch_times, os.path.join(results_path, f"epoch_times_{cell}")
        )

        torch.save(
            plot_train, os.path.join(results_path, "train_loss_{}".format(cell))
        )
        torch.save(
            plot_valid,
            os.path.join(results_path, "avg_valid_loss_{}".format(cell)),
        )
        ax.plot(
        ...
        for idx, (
            ...
            test_loss = loss_fn(test_output, target12_test, target21_test)
        ...
        torch.save(
            test_truelist12,
            os.path.join(results_path, "true_match12_{}".format(cell)),
        )
        torch.save(
            test_truelist21,
            os.path.join(results_path, "true_match21_{}".format(cell)),
        )
        torch.save(
            test_predlist,
            os.path.join(results_path, "pred_match_{}".format(cell)),
        )
        ...
        torch.save(
            test_lengths,
            os.path.join(results_path, "test_lengths_{}".format(cell)),
        )
    ...
    fig.savefig(
        os.path.join(
            results_path, "setmatchloss_{}.png".format(params["loss"])
        ),
        bbox_inches="tight",
    )
