...
from fdsa.utils.layers.peephole_lstm import PeepholeLSTMCell
...
def test_lstm_cell():
    ...
    for bias in [True, False]:
        ...
        lstm = PeepholeLSTMCell(input_size, hidden_size)
        ...
        assert lstm.weights_c.size() == torch.Size([hidden_size, hidden_size * 3])
        assert lstm.weights_h.size() == torch.Size([hidden_size, hidden_size * 4])
        assert lstm.weights_x.size() == torch.Size([input_size, hidden_size * 4])
